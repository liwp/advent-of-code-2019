...
(defn in-between? [a x b]
  (let [[a b] (if (< a b) [a b] [b a])]
    (<= a x b)))

(defprotocol PSegment
  (segment-distance [_ point])
  (intersects [_ other]))

(defrecord Horizontal [x-a x-b y]
  PSegment

  (segment-distance [_ point]
    (let [hit (and (= y (:y point)) (in-between? x-a (:x point) x-b))
          x-b (if hit (:x point) x-b)]
      {:hit hit :d (Math/abs (- x-b x-a))}))

  (intersects [_ other]
    (if (instance? Horizontal other)
      false
      (let [{:keys [y-a y-b x]} other]
        (and (in-between? y-a y y-b)
             (in-between? x-a x x-b))))))

(defrecord Vertical [y-a y-b x]
  PSegment

  (segment-distance [_ point]
    (let [hit (and (= x (:x point)) (in-between? y-a (:y point) y-b))
          y-b (if hit (:y point) y-b)]
      {:hit hit :d (Math/abs (- y-b y-a))}))

  (intersects [_ other]
    (if (instance? Vertical other)
      false
      (let [{:keys [x-a x-b y]} other]
        (and (in-between? y-a y y-b)
             (in-between? x-a x x-b))))))
...
(defn parse-wire [wire-str]
  (->> wire-str
       (re-seq #"(.)(\d+),?")
       (map (fn [[_ dir length]] {:dir (keyword dir) :length (read-string length)}))))

(defn wire->segments [wire]
  (loop [segments [] {:keys [x y]} {:x 0 :y 0} [{:keys [dir length]} & wire] wire]
    (if dir
      (let [[segment x y]
            (case dir
              :U [(->Vertical y (+ y length) x) x (+ y length)]
              :D [(->Vertical y (- y length) x) x (- y length)]
              :R [(->Horizontal x (+ x length) y) (+ x length) y]
              :L [(->Horizontal x (- x length) y) (- x length) y])]
        (recur (conj segments segment) {:x x :y y} wire))
      segments)))

(defn parse [wire-str]
  (-> wire-str parse-wire wire->segments))

(def wires (map parse input))

(defn intersection-point [a b]
  (when (.intersects a b)
    (let [[h v] (if (instance? Horizontal a) [a b] [b a])]
      {:x (:x v) :y (:y h)})))

(defn intersections [segs-1 segs-2]
  (for [a segs-1
        b segs-2
        :let [point (intersection-point b a)]
        :when (and point (not= point {:x 0 :y 0}))]
    point))

(defn manhattan-distance [{:keys [x y]}]
  (+ (Math/abs x) (Math/abs y)))

(defn distance-to-closest-intersection-point [ps]
  (apply min (map manhattan-distance ps)))

(defn distance-to-intersection-point [wire point]
  (loop [distance 0 [seg & wire] wire]
    (if (not seg)
      distance
      (let [{:keys [hit d]} (.segment-distance seg point)]
        (recur (+ distance d ) (if hit nil wire))))))

(defn combined-distance-to-point [wires point]
  (reduce + (map #(distance-to-intersection-point % point) wires)))

(defn run-part-1 [wires]
  (->> wires
       (apply intersections)
...
(defn run-part-2 [wires]
  (->> wires
       (apply intersections)
       (map #(combined-distance-to-point wires %))
       (apply min)))
