...
(fn m.position [droid direction]
  (let [N 1 S 2 W 3 E 4]
    (match direction
      N {:x droid.position.x :y (- droid.position.y 1)}
      S {:x droid.position.x :y (+ droid.position.y 1)}
      E {:x (+ droid.position.x 1) :y droid.position.y}
      W {:x (- droid.position.x 1) :y droid.position.y})))

(fn m.opposite [direction]
  (. {1 2
      2 1
      3 4
      4 3}
...
(fn m.move [droid direction found]
  (let [position (m.position droid direction)
        position-hashed (m.hash-point position)]
    (tset droid :position position)
    (tset droid.map position-hashed found)
    (tset droid :bounds {:w (math.min droid.bounds.w position.x)
                         :e (math.max droid.bounds.e position.x)
                         :n (math.min droid.bounds.n position.y)
                         :s (math.max droid.bounds.s position.y)})
    (when (not (. droid.options position-hashed))
      (tset droid.options position-hashed (m.remove #(= $ (m.opposite direction)) [1 2 3 4]))
      (table.insert droid.path direction))))

(fn m.wall [droid direction]
  (let [wall (m.position droid direction)]
    (tset droid.map (m.hash-point wall) "#")
    droid))

(fn m.draw [{: map : position : bounds}]
  (print)
  (for [y (- bounds.n 1) (+ bounds.s 1)]
    (var row "")
    (for [x (- bounds.w 1) (+ bounds.e 1)]
      (set row (.. row (if (and (= position.x x) (= position.y y))
                           "D"
                           (or (. map (m.hash-point {: x : y})) " ")))))
    (print row)))

(fn m.droid-step [[_ droid xs ip rb] direction]
  (let [[_ xs ip rb in [status]] (c.run-loop [:ok xs ip rb [direction] []])]
    (match status
      0 (m.wall droid direction)
      1 (m.move droid direction ".")
      2 (m.move droid direction "X"))
    [status droid xs ip rb]))

(fn m.find-oxygen [[status droid xs ip rb]]
  (if (= 2 status)
      (length droid.path)
      (let [options (. droid.options (m.hash-point droid.position))
            d (or (table.remove options)
                  (m.opposite (table.remove droid.path)))]
        (m.find-oxygen (m.droid-step [status droid xs ip rb] d)))))

(fn m.everywhere [[status droid xs ip rb]]
  (let [options (. droid.options (m.hash-point droid.position))
        d (or (table.remove options)
              (m.opposite (table.remove droid.path)))]
    (if d
        (m.everywhere (m.droid-step [status droid xs ip rb] d))
        droid)))

(fn m.neighbours [map pt]
  (let [t []]
    (each [_ n (pairs [{:x (+ pt.x 1) :y pt.y}
                       {:x (- pt.x 1) :y pt.y}
                       {:x pt.x :y (+ pt.y 1)}
                       {:x pt.x :y (- pt.y 1)}])]
      (when (= (. map (m.hash-point n)) ".")
        (table.insert t n)))
    t))

(fn m.oxygen-spread [count map]
  (var empty 0)
  (each [k v (pairs (m.clone map))]
    (if (= v ".")
        (set empty (+ empty 1))
        (= v "X")
        (each [_ n (pairs (m.neighbours map (m.unhash-point k)))]
          (tset map (m.hash-point n) "X"))))
  (if (= empty 0)
      count
      (m.oxygen-spread (+ count 1) map)))

(print (.. "Day 15 / Part 1: "
           (m.find-oxygen [nil (m.init-droid) (c.zero-base m.input) 0 0])))

(print (.. "Day 15 / Part 2: "
           (let [{: map} (m.everywhere [nil (m.init-droid) (c.zero-base m.input) 0 0])]
             (m.oxygen-spread 0 map))))
